...
shinyServer(
  ...
  function(input,output)
  {
    data <- reactive({
      ...
      #con <- mongo(collection = "train", url = "mongodb://abhishek_1812:abhishek1812@ds031671.mlab.com:31671/shiny")
   ...
   ###################################################################################################################################### 
    ...
    output$tb <- renderUI({
      if(is.null(data()))
        ...
        {
        tabsetPanel(
          tabPanel("  ", verbatimTextOutput("filedf")),
          tabPanel("Artifical Neural Networks (ANN)", plotOutput("plotann"),  tableOutput("sample"), tableOutput("ann"), tableOutput("table")),
          tabPanel("  ", tableOutput("svm"),  tableOutput("sample1"), tableOutput("table1")),
          tabPanel("  ", tableOutput("sda"), tableOutput("sample2"),tableOutput("table2")),
          tabPanel("Radial Basic Function (RBF)", tableOutput("rbf"), plotOutput("rbfplot"), tableOutput("sample3"), tableOutput("table3")),
          tabPanel("  ", tableOutput("lda"), plotOutput("plotlda"), tableOutput("sample4"), tableOutput("table4")),
          tabPanel("  ", verbatimTextOutput("crtext"), tableOutput("cr1"), tableOutput("cr2"), tableOutput("cr3"), verbatimTextOutput("cr2text"), tableOutput("crr1"), tableOutput("crr2"), tableOutput("crr3")),
          tabPanel("  ", verbatimTextOutput("braintext"), plotOutput("brainplot")))
        }
        else if(input$f64 == TRUE)
        {
          tabsetPanel(
            ...
            tabPanel("  ", tableOutput("svm64"),  tableOutput("sample164"), tableOutput("table164")),
            tabPanel("  ", tableOutput("sda64"), tableOutput("sample264"),tableOutput("table264")),
            tabPanel("Radial Basic Function (RBF)", tableOutput("rbf64"), plotOutput("rbfplot64"), tableOutput("sample364"), tableOutput("table364")),
            tabPanel("  ", tableOutput("lda64"), plotOutput("plotlda64"), tableOutput("sample464"), tableOutput("table464")),
            tabPanel("  ", verbatimTextOutput("crtext"), tableOutput("cr1"), tableOutput("cr2"), tableOutput("cr3"), verbatimTextOutput("cr2text"), tableOutput("crr1"), tableOutput("crr2"), tableOutput("crr3")),
          tabPanel("  ", verbatimTextOutput("braintext64"), plotOutput("brainplot64")))
        }
      }
    })
